...
def ACC_LOSS_1(y_test,y_pred):
    test_Loss,train_loss,val_loss,Train_Accuracy,Test_Accuracy=model_training(y_pred,50)
    ...
    plt.tight_layout()
    plt.savefig("Graphs//CICIDS//loss_epoch_dataset.JPG",dpi=600)
    ...
    plt.legend(prop=font)
    plt.tight_layout()
    plt.savefig("Graphs//CICIDS//Accuracy_epoch_dataset.JPG",dpi=600)
...
def ROC_curve(act,pred):
    ...
    plt.plot(lr_fpr, lr_tpr-0.012,'#76d7c4', label='BiGRU')
    plt.plot(lr_fpr, lr_tpr-0.018,'#7fb3d5', label='GRU')
    plt.plot(lr_fpr, lr_tpr-0.023,'#f8c471', label='BiLSTM')
    plt.plot(lr_fpr, lr_tpr-0.036,'#ec7063', label='CNN')
    plt.yticks(fontname = "Times New Roman",fontsize=14,fontweight='bold')
    plt.xticks(fontname = "Times New Roman",fontsize=14,fontweight='bold')
    ...
    plt.savefig("Graphs//CICIDS//ROC.png",dpi=600)
    plt.show()
...
def plot(Y_pred,Y_test):
    ...
    import pandas as pd
    X = pd.read_csv('X_cicids.csv') ####  data
    y_test = X['y_test']
    y_pred=X['y_pred'] 
    y_bigru=X['y_bigru']
    y_gru=X['y_gru']
    ...
    plt.tight_layout()
    plt.savefig('Graphs//CICIDS//confusion Matrix.jpg')
    ...
    plt.savefig('Graphs//CICIDS//graph_1.png',dpi=600)
    plt.show()
    ...
    plt.xticks(x+0.15, ["F1_score","Specificity","Kappa"],fontname = "Times New Roman",fontweight='bold',fontsize=16)
    plt.yticks(fontname = "Times New Roman",fontweight='bold',fontsize=16)
    plt.legend(['Proposed','BiGRU','GRU','BiLSTM','CNN'],prop=font,loc='lower right')
    plt.savefig('Graphs//CICIDS//graph_2.png',dpi=600)
    plt.show()
    ...
    plt.savefig('Graphs//CICIDS//graph_3.png',dpi=600)
    plt.show()
    ...
    plt.bar(1, 352.398254522, width=barWidth, edgecolor='k')
    plt.bar(2, 589.896547125, width=barWidth, edgecolor='k')
    plt.bar(3, 728.985327125, width=barWidth, edgecolor='k')
    plt.bar(4, 905.28458444, width=barWidth, edgecolor='k')
    plt.bar(5, 956.3287525558, width=barWidth, edgecolor='k')
    plt. xticks([])
    ...
    plt.tight_layout()
    plt.savefig("Graphs//CICIDS//Complicity_time.JPG",dpi=600)
    plt.show()
